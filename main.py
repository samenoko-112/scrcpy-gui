...
def main(page:Page):
    ...
    def check_audio_and_video(e):
        if no_audio.value == True and no_video.value == True:
            page.snack_bar = SnackBar(Text("エラー: 音声・映像両方を無効にすることはできません"), bgcolor=colors.RED)
    ...
    def scan_devices(e):
        try:
            ...
            page.snack_bar = SnackBar(Text(f"エラー: {e}"), bgcolor=colors.RED)
            page.snack_bar.open = True
            page.update()
    ...
    def run_scrcpy(e):
        ...
        if device_list.value == None:
            page.snack_bar = SnackBar(Text("エラー: デバイスが選択されていません"), bgcolor=colors.RED)
            page.snack_bar.open = True
            page.update()
            return
        ...
        if device_id:
            ...
            try:
                ...
                page.snack_bar = SnackBar(Text(f"Error: {e}"), bgcolor=colors.RED)
                page.snack_bar.open = True
                page.update()
                current_process = None
                process_running = False
